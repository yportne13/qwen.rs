...
fn main() -> Result<(), Error> {
    ...
    let dtype = if buf.strip_prefix('0').is_some() {
        0
    ...
    ///////
    fn write_2dim(vb: &VarBuilder, file: &mut std::fs::File, size: (u32, u32), name: &str, dtype: u32) -> Result<(), Error> {
        file.write_all(&u32_to_array_of_u8(2))?;//dims
        file.write_all(&u32_to_array_of_u8(name.len() as u32))?;//num_len
        match dtype {
            0 => {file.write_all(&[1, 0, 0, 0])?;},//dtype F16
            2 => {file.write_all(&[2, 0, 0, 0])?;},//dtype q4_0
            6 => {file.write_all(&[6, 0, 0, 0])?;},//dtype q5_0
            _ => {file.write_all(&[8, 0, 0, 0])?;},//dtype q8_0
        }
    ...
    write_2dim(&vb, &mut ggml, (151936, 4096), "transformer.wte.weight", dtype)?;
    for l_idx in 0..32 {
        write_2dim(&vb, &mut ggml, (4096*3, 4096), &format!("transformer.h.{l_idx}.attn.c_attn.weight"), dtype)?;
        write_1dim(&vb, &mut ggml, 4096*3, &format!("transformer.h.{l_idx}.attn.c_attn.bias"))?;
        write_2dim(&vb, &mut ggml, (4096, 4096), &format!("transformer.h.{l_idx}.attn.c_proj.weight"), dtype)?;
        write_2dim(&vb, &mut ggml, (11008, 4096), &format!("transformer.h.{l_idx}.mlp.w1.weight"), dtype)?;
        write_2dim(&vb, &mut ggml, (11008, 4096), &format!("transformer.h.{l_idx}.mlp.w2.weight"), dtype)?;
        write_2dim(&vb, &mut ggml, (4096, 11008), &format!("transformer.h.{l_idx}.mlp.c_proj.weight"), dtype)?;
    ...
    write_2dim(&vb, &mut ggml, (151936, 4096), "lm_head.weight", dtype)?;
